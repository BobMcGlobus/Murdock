...
from voiceid.core.info_cache import UpstreamInfoCache
from voiceid.core.liveness import analyze_pcm as analyze_liveness
from voiceid.core.recognition_log import (
...
_LOGGER = logging.getLogger("voiceid.wyoming.handler")
...
class VoiceIDHandler(AsyncEventHandler):
    ...
    async def _read_upstream_transcript(self) -> None:
        ...
        obvious in the VoiceID logs instead of manifesting as a silent
        empty transcript.
        """
        sid = self._session_id
    ...
    async def _run_early_probe(self, audio_snapshot: bytes) -> None:
        ...
        sid = self._session_id
        try:
            from voiceid.core.audio import to_mono_16k_pcm
