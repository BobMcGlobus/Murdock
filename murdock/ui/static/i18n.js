...
const I18N = (() => {
    const TRANSLATIONS = {
        en: {
            ...
            "transcript_tpl.description": "How the recognised speaker reaches your conversation agent. Pick the mode that matches how you run Assist.",
            "transcript_tpl.mode": "Delivery mode",
            "transcript_tpl.mode_none": "MQTT / system prompt — keeps local intents",
            "transcript_tpl.mode_transcript": "Transcript augmentation — fresh per turn, LLM only",
            "transcript_tpl.help_none": "Murdock returns the raw transcript untouched. The speaker is published via MQTT sensors (and/or REST), which you read in the HA conversation agent's system prompt. HA's local intent matching (\"turn on the light\") keeps working. Caveat: HA caches the system prompt within a conversation, so the speaker value can lag if two people talk in quick succession — shorten the conversation lifetime to mitigate.",
            "transcript_tpl.help_transcript": "Murdock injects the speaker context straight into the transcript it returns, using the templates below — so it arrives fresh on every single utterance, no MQTT needed and no caching staleness. Downside: HA's built-in local intent matching will no longer match (the command text is no longer clean), so this is for setups where an LLM handles every command.",
        ...
        de: {
            ...
            "transcript_tpl.description": "Wie der erkannte Sprecher beim Konversationsagenten ankommt. Wähle den Modus passend zu deinem Assist-Setup.",
            "transcript_tpl.mode": "Übergabe-Modus",
            "transcript_tpl.mode_none": "MQTT / System-Prompt — lokale Intents bleiben",
            "transcript_tpl.mode_transcript": "Transkript-Anreicherung — frisch pro Turn, nur LLM",
            "transcript_tpl.help_none": "Murdock gibt das Transkript unverändert zurück. Der Sprecher wird über MQTT-Sensoren (und/oder REST) veröffentlicht, die du im System-Prompt deines HA-Konversationsagenten ausliest. HAs lokales Intent-Matching (\"mach das Licht an\") funktioniert weiter. Haken: HA cacht den System-Prompt innerhalb einer Konversation, der Sprecher-Wert kann also nachhängen, wenn zwei Personen kurz hintereinander sprechen — kürzere Konversations-Lebensdauer mildert das.",
            "transcript_tpl.help_transcript": "Murdock spielt den Sprecher-Kontext direkt ins zurückgegebene Transkript ein (Vorlagen unten) — frisch bei jeder einzelnen Äußerung, ohne MQTT und ohne Cache-Problem. Nachteil: HAs eingebautes lokales Intent-Matching greift nicht mehr (der Befehlstext ist nicht mehr sauber); also für Setups, in denen ein LLM jeden Befehl bearbeitet.",
