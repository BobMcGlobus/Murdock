...
languages advertised in the returned ``Info`` event. If Murdock serves a
...
_LOGGER = logging.getLogger("murdock.info_cache")
...
class UpstreamInfoCache:
    ...
    async def prime(self, attempts: int = 5, delay: float = 3.0) -> None:
        ...
        _LOGGER.warning(
            ...
            "Configure an override in the Murdock settings UI to unblock HA.",
            attempts,
        )
        ...
        return Info(
            asr=[
                AsrProgram(
                    name="murdock",
                    description=(
                        f"Murdock v{__version__} — speaker-gated ASR proxy"
                    ),
